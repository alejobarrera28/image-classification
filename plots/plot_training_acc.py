...
def plot_training_accuracy(model_name):
    """
    Generate and save training/validation accuracy plot for a given model.
    ...
    plt.title(
        f"{model_name.upper()} - Training and Validation Accuracy",
        fontsize=14,
        fontweight="bold",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate training accuracy plot for a model"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    plot_training_accuracy(args.model)
